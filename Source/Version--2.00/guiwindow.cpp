...
void guiWindow::PortsSearch()
{
    ...
    if (serialFoundList.isEmpty()) {
        ...
        // If no devices found, show a user-friendly error message
        PopupWindow("No devices detected!", "Make sure the microcontroller board running P.I.G.S is connected and recognized by the PC. P.I.G.S-GUI will now close.", "ERROR", 4);
        return;  // Simply return instead of calling exit()
    } else {
        QList<QString> piggieDevices;

        for (const QSerialPortInfo &port : serialFoundList) {
            int vendorId = port.vendorIdentifier();
            int productId = port.productIdentifier();
            QString comPort = port.systemLocation();

            qDebug() << "Checking port: VID =" << vendorId << " PID =" << productId;

            QString piggieName = "NOT A PIGS DEVICE";  // Default name for non-Piggie devices
            // Match the vendorId and productId to the correct Piggie name
            if (vendorId == 0x0321) {  // VID: 0322
                switch (productId) {
                case 0x0420: piggieName = "Piggie 1"; break;
                }
            }
            else if (vendorId == 0x0322) {  // VID: 0321
                switch (productId) {
                case 0x0421: piggieName = "Piggie 2"; break;
                }
            }
            else if (vendorId == 0x0323) {  // VID: 0323
                switch (productId) {
                case 0x0422: piggieName = "Piggie 3"; break;
                }
            }
            else if (vendorId == 0x0324) {  // VID: 0324
                switch (productId) {
                case 0x0423: piggieName = "Piggie 4"; break;
                }
            }
            // Clean the COM Port format (remove "\\.\")
            QString cleanedComPort = comPort;
            cleanedComPort.remove("\\\\.\\");  // Remove "\\.\"

            // Add the device to piggieDevices list
            piggieDevices.append(QString("%1 %2").arg(piggieName).arg(cleanedComPort));
        }

        // Check if no Piggie devices were found
        if (piggieDevices.isEmpty()) {
            qDebug() << "No Piggie devices detected!";
            PopupWindow("No P.I.G.S devices detected!", "Make sure the microcontroller board running P.I.G.S is connected and recognized by the PC. P.I.G.S-GUI only supports firmwares starting from v4.0 \"L.G.A Force\" and onwards.", "ERROR", 4);
            return;  // Do not call exit(), just return
        }

        // Assign the list of detected Piggie devices
        usbName = piggieDevices;

        // Debug output to verify devices found
        qDebug() << "Found Piggie devices:\n" << usbName.join("\n");

        // Optionally, update the UI with the list of found devices
        // ui->comboBox->clear();  // For example, clear the combobox
        // ui->comboBox->addItems(usbName);  // Populate combobox with the devices
    }
}
    ...
    , ui(new Ui::guiWindow)
{
    ...
    PortsSearch();
    usbName.prepend("[Pick LightGun Here]");
